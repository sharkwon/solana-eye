...
- **Validators** — active/delinquent counts, stake distribution, top 20 by stake,
  average commission, delinquency alerts
...
│  └─ dune.py       optional Dune dashboards (needs DUNE_API_KEY) │
│                                                                │
...
[workflow](.github/workflows/refresh.yml) re-runs the pipeline hourly and publishes the
result to GitHub Pages automatically. Enable Pages → *Deploy from branch → main → /docs*.
...
|---|---|---|---|
| Solana RPC | `api.mainnet-beta.solana.com` | no | health, epoch, TPS samples, vote accounts, supply, fee sampling |
...
| Dune (optional) | `api.dune.com/api/v1` | `DUNE_API_KEY` env | sponsor-priority dashboards (REV, DAU, tokenized equities) |

**Design decisions:**

- **Keyless by default.** The public Solana RPC + free analytics APIs cover every
  headline metric with no signup. A custom RPC URL can be swapped in `config.json`
  (`rpc.url`) for heavier deployments; Dune bolts on via an env var when enabled.
...
1. **Threshold rules** (domain knowledge): slot time > 0.6s · delinquent stake > 5% ·
   TVL/price/DEX-volume moves beyond ±5% in 24h · status-page incidents.
2. **Statistical z-scores**: current value vs. the metric's own history (min 5 samples,
...
(no CDN, works offline). KPI cards with sparklines, anomaly banner, top-validators
table, collapsible SIMD news, source health table.
...
Structured machine-readable snapshot (`schema_version: 1`): network, validators, fees,
economics, news, anomalies, history (last 24 snapshots for downstream tooling).
...
MIT — see [LICENSE](LICENSE). Data © respective sources (Solana Labs, DeFiLlama,
